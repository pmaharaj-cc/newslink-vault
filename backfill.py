"""
NewsLink Backfill Script
Run: python3 backfill.py
Reprocesses all existing articles with the latest Groq prompt
(role inference + author exclusion + criminal tracking).
"""
import json, base64, urllib.request, re, time, os
...
def safe(name): return re.sub(r'[<>:"/\\|?*]', '', str(name)).strip()
def wl(name): return f"[[{safe(name)}]]"
...
def fetch_text(url):
    try:
        ...
        # Target asset-content div for clean article body (excludes nav/ads/related)
...
def groq_extract(text):
    payload = json.dumps({"model": "llama-3.1-8b-instant", "temperature": 0, "max_tokens": 1024,
...
def build_note(d, url, pub_date):
    ...
    orgs, places, topics = d.get("organizations") or [], d.get("places") or [], d.get("topics") or []
    ...
    if authors: lines += [f"**By:** {' · '.join(f'[[Authors/{safe(a)}|{a}]]' for a in authors)}", ""]
    if date_eff and date_eff != date: lines += [f"> ⚠️ **Effective:** {date_eff} (reported {date})", ""]
    if people:
        parts = []
        for p in people:
            s = f"[[People/{safe(p['name'])}|{p['name']}]]"
            if p.get("role"): s += f" _({p['role']})_"
            if p.get("legal_status"): s += f" ⚠️ _{p['legal_status']}_"
            parts.append(s)
        lines += ["## People", " · ".join(parts), ""]
    if orgs: lines += ["## Organizations", " · ".join(f"[[Orgs/{safe(o)}|{o}]]" for o in orgs), ""]
    if places: lines += ["## Places", " · ".join(f"[[Places/{safe(p)}|{p}]]" for p in places), ""]
    if topics: lines += ["## Topics", " · ".join(f"[[Topics/{safe(t)}|{t}]]" for t in topics), ""]
    ...
    if rels:
        lines += ["## Relationships", ""]
        for r in rels: lines.append(f"- {wl(r['from'])} **{r['relation']}** {wl(r['to'])}")
        lines.append("")
    quotes = d.get("quotes") or []
    if quotes:
        lines += ["## Key Quotes", ""]
        for q in quotes: lines += [f'> "{q["text"]}"', f'> — {wl(q["speaker"])}', ""]
...
def build_criminal_stub(name, data):
    ...
    if roles: lines += [f"**Known roles:** {', '.join(roles)}", ""]
...
for i, entry in enumerate(entries):
    ...
    for p in (d.get("people") or []):
        ...
        name = p["name"]
        if name not in entities["People"]: entities["People"][name] = {"roles":[],"statuses":[],"articles":[]}
...
new_commit = gh("git/commits", "POST", json.dumps({
    "message": "backfill: role inference + author exclusion on all articles",
